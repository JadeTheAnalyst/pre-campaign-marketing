...
INNER JOIN digital_marketing.orders o ON o.order_customer_id=c.customer_id
WHERE o.order_status NOT IN ('ON_HOLD',
                             'CANCELED',
                             'SUSPECTED_FRAUD')
...
####another solution: do each city by each city
DROP
TEMPORARY TABLE IF EXISTS digital_marketing.final_list1;
CREATE
TEMPORARY TABLE IF NOT EXISTS digital_marketing.final_list1
as (select * from digital_marketing.final_list where customer_city='Bronx' limit 2);

DROP
TEMPORARY TABLE IF EXISTS digital_marketing.final_list2;
CREATE
TEMPORARY TABLE IF NOT EXISTS digital_marketing.final_list2
as (select * from digital_marketing.final_list where customer_city='Brooklyn' limit 2)
;

select * from digital_marketing.final_list1
union 
select * from digital_marketing.final_list2;
